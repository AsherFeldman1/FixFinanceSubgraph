...
import { getUser, BigZero, handleChange2, getVault, getYTVault, handleYTChange2 } from '../helpers'
...
export function handleOpenVault(event: OpenVault): void {
  let id = (event.params.index).toHexString().concat("-")
	.concat("2")
  handleChange2(event.params.index, (event.params.owner).toHexString(), id, (event.address).toHexString())
}

export function handleCloseVault(event: CloseVault): void {
  let id = (event.params.index).toHexString().concat("-")
	.concat("2")
  let vault = getVault(id, (event.params.owner).toHexString(), 2)
  vault.Debt = ""
  vault.Collateral = ""
  vault.Owner = ""
  vault.save()
...
export function handleAdjustVault(event: AdjustVault): void {
  let id = (event.params.index).toHexString().concat("-").concat("2")
  handleChange2(event.params.index, (event.params.owner).toHexString(), id, (event.address).toHexString())
}
...
export function handleOpenYTVault(event: OpenYTVault): void {
  let id = (event.params.index).toHexString().concat("-")
  .concat("2")
  handleYTChange2(event.params.index, (event.params.owner).toHexString(), id, (event.address).toHexString())
}

export function handleCloseYTVault(event: CloseYTVault): void {
  let id = (event.params.index).toHexString().concat("-")
  .concat("2")
  let vault = getYTVault(id, (event.params.owner).toHexString(), 2)
  vault.Debt = ""
  vault.Collateral = ""
  vault.Owner = ""
  vault.save()
...
export function handleTransferVault(event: TransferVault): void {
  ...
  if (event.params.isYTVault) {
    let vault = getYTVault(id, (event.params.prevOwner).toHexString(), 2)
    vault.Owner = (event.params.newOwner).toHexString()
    vault.save()
  } else {
    let vault = getVault(id, (event.params.prevOwner).toHexString(), 2)
    vault.Owner = (event.params.newOwner).toHexString()
    vault.save()
  }
}